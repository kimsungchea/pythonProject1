...
def feature_engineering():
    ...
    train['weather'] = train['weather'].map({1: 'clear',
                                             ...
                                             4: 'heavey rain, thunderstorm, snow, fog'})
...
if mnu == '설명':
    ...
    st.markdown('**registered** - 등록된 않은 사용자 (회원) 수')
    ...
    train['weather'] = train ['weather'].map({1: 'clear' 
                                              2: 'mist, few clouds',
                                              3: 'light snow, rain, thunderstorm',
                                              4. 'heavy rain, thunderstorm, snow, fod'})
    train['weekdat'] = train['date'].apply(
        lambda date: calendar.day_name [datetime.combine(date, datetime.min.time ()).weekday()] 
    )
    ''')
    ...
    mpl.rc('font', size=12)
    plt, ((ax1, ax2), (ax3, ax4)) = plt.subplots(nrows=2, ncols=2)
    plt.tight_layout()
    ...
    st.markdown('**2. 파생피처 추가:** datetime 피처는 여러 가지 정보의 혼합체이므로 '
                '각각을 분리해 year, month, dat,hour,minute,second 피쳐를 생성할 수 있다')
    ...
    st.markdown('**7.  피처 제거:** month는 season 피처의 세부 분류로 볼 수 있다.'
                '데이터가 지나치가 세분화되어 있으면 분류별 데이터수가 적어서 학습에 오히려 방해가 되기도 한다.')
    ...
    st.markdown('[위키피디아: 선형회귀](https://ko.wikipedia.org/wiki/%EC%84%A0%ED%98%)')
    ...
    linear_reg_model = Linear Regression()
    ...
    st.write(f' 선형 회귀의 RMSLE: {rmsle_value:.4f}')
